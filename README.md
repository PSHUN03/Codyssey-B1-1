...
> **결론** — 환각 측면에서 대체로 우수하며, 발견된 문제는 모두 **신규 규칙 없이 기존 설계 조항으로 재발 방지가 가능**하다. 다만 계층 형식·톤 범위 미충족 2건은, 로그에 §2.5 시스템 프롬프트가 적용된 정황이 없다는 점에서 **설계의 실패가 아니라 설계의 필요성을 입증하는 대조군(baseline)** 으로 읽는 것이 타당하다.
